...
int SRAEditCasesPopulate ( SRAModel * SRAMismatchModel, int nextCase,
                           unsigned int ReadLength, unsigned char MaxError, uint8_t smallErrorFirst,
                           BWT * bwt, BWT * rev_bwt )
{
...
int SRANBMismatchCasesPopulate(SRAModel * SRAMismatchModel, int nextCase, 
                             unsigned int ReadLength, int ReadStrand, unsigned char MaxError, uint8_t smallErrorFirst, 
                              BWT * bwt, BWT * rev_bwt) {
    ...
    if ( ErrorType == SRA_STEP_ERROR_TYPE_NO_ERROR || ErrorType == SRA_STEP_ERROR_TYPE_MISMATCH_ONLY )
    {
        if ( modelId == SRA_MODEL_8G )
        {
            ...
            if (aSettings->MaxNBMismatch>0 && MaxError<=1) {
                j=SRANBMismatchCasesPopulate(SRAMismatchModel,j,ReadLength,ReadStrand,MaxError,SmallErrorFirst,bwt,rev_bwt);
            } else {
