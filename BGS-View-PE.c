...
int main ( int argc, char ** argv )
{
    if ( argc < 3 )
    {
        ...
        // fprintf(stderr,"Windows Platform:\n\t BGS-View-PE <query_file_1> <query_file_2> <output_file>\n");
    ...
    //Open the necessary files for decoding
    // fprintf(stderr, "Opening %s .. ",readFileName2);
    FILE * queryFile2 = ( FILE * ) fopen ( readFileName2, "r" );

    if ( queryFile2 == NULL || queryFile2 == 0 )
    {
        fprintf ( stderr, "Opening %s failed!\n", readFileName2 );
        return 0;
    }
    ...
    char * queryFileBuffer2 = ( char * ) malloc ( sizeof ( char ) * BGS_VIEWER_INPUT_BUFFER_SIZE );
    size_t bufferSize2 = fread ( queryFileBuffer2, sizeof ( char ), BGS_VIEWER_INPUT_BUFFER_SIZE, queryFile2 );
    unsigned int bufferIndex2 = 0;
    char queryChar2 = queryFileBuffer2[bufferIndex2++];
    int isFastq = 0;
    ...
    while ( bufferSize != 0 && bufferSize2 != 0 )
    {
        ...
        while ( bufferSize2 != 0 && queryChar2 != '>' && queryChar2 != '@' )
        {
            ...
            {
                bufferSize2 = fread ( queryFileBuffer2, sizeof ( char ), BGS_VIEWER_INPUT_BUFFER_SIZE, queryFile2 );
                bufferIndex2 = 0;
            }
        }
        ...
        while ( bufferSize2 != 0  && queryChar2 != '\n' )
        {
            ...
            if ( !cropped && headerCharId < BGS_VIEWER_QUERY_NAME_LENGTH )
            {
                upkdQueriesName[queriesRead * ( BGS_VIEWER_QUERY_NAME_LENGTH + 1 ) + ( headerCharId++ )] = queryChar2;
            }
        ...
        while ( bufferSize2 != 0  && queryChar2 != '>' && queryChar2 != '@' && queryChar2 != '+' )
        {
            if ( queryChar2 != '\n' )
            {
                if ( nucleoId < maxReadLength )
                {
                    upkdQueries[queriesRead * ( maxReadLength + 1 ) + nucleoId] = queryChar2;
                }
    ...
    free ( queryFileBuffer2 );
    fclose ( queryFile );
    fclose ( queryFile2 );
    // fprintf(stderr,"[Main] %lld short reads loaded from two query files\n", queriesRead); numQueries=queriesRead;
    ...
    while ( i < outputNumberOfRecords )
    {
        ...
        {
            ReadId = cacheFlag3;
            alignedRead[0] = upkdQueries + ( ReadId - 2 ) * ( maxReadLength + 1 );
            alignedReadHeader[0] = upkdQueriesName + ( ReadId - 2 ) * ( BGS_VIEWER_QUERY_NAME_LENGTH + 1 );
            alignedRead[1] = upkdQueries + ( ReadId - 1 ) * ( maxReadLength + 1 );
            alignedReadHeader[1] = upkdQueriesName + ( ReadId - 1 ) * ( BGS_VIEWER_QUERY_NAME_LENGTH + 1 );
        }
        else
        {
            int isRead2 = ( numberOfAlignments % 2 ); // 0: first read 1: second read
            querySequenceLength = GetStringLength ( alignedRead[isRead2], maxReadLength );
            //get read
            ChromId = cacheFlag2 - 1;
            tp = cacheFlag3;
            ReadStrand = cacheFlag1;
            printf ( "%llu\t%s\t%s\t", ( ReadId + 1 ) / 2, alignedReadHeader[isRead2], alignedRead[isRead2] );
