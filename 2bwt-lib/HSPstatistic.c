...
double stat_log2;
int32_t stat_ungapXdropoff;
int32_t stat_gapXdropoff;
int32_t stat_gapXdropoffFinal;
...
uint64_t stat_databaseSize;
uint64_t stat_numOfSequences;
uint64_t stat_averageSubjectSize; // average sequence size in database
uint64_t stat_minimumSubjectSize; // minimum sequence size in database
int64_t stat_adjustmentLength;
uint64_t stat_querySize;
uint64_t stat_numOfContext;
uint64_t stat_effectiveQuerySize;
uint64_t stat_effectiveDatabaseSize;
uint64_t stat_effectiveSearchSpaceSize;
...
/*  forward declaration */
void stat_calcUngapKarlinParameters(double *dbCharProb, double *queryCharProb, int scoringMatrix[DNA_CHAR_SIZE][DNA_CHAR_SIZE]);
                            ...
                            uint64_t querySize, uint64_t numOfContext, double *queryCharProb, int scoringMatrix[DNA_CHAR_SIZE][DNA_CHAR_SIZE])
{
  ...
  stat_averageSubjectSize = (uint32_t)(databaseSize /numOfSequences);  
  ...
  stat_ungapXdropoff = (int32_t)ceil(stat_XdropoffsInBits.ungapXdropoff * stat_log2 /
                                  stat_ungapLambda); 
  stat_gapXdropoff = (int32_t)floor(stat_XdropoffsInBits.gapXdropoff * stat_log2 /
                                 stat_gapLambda); 
  stat_gapXdropoffFinal = (int32_t)floor(stat_XdropoffsInBits.gapXdropoffFinal * 
  ...
  stat_adjustmentLength = (int32_t)floor(stat_calcLengthAdjust());
...
void stat_calcUngapKarlinParameters(double *dbCharProb, double *queryCharProb, int scoringMatrix[DNA_CHAR_SIZE][DNA_CHAR_SIZE])
{
  int32_t scoreProbSize, highest, lowest;
  double * scoreProb;
  int i, j;
...
{
  uint64_t minSize = stat_minimumSubjectSize;
  int32_t gappedScore, ungappedScore;

  if (minSize > stat_querySize)
     minSize = stat_querySize;

  ungappedScore = (int32_t)ceil(
...
int32_t stat_ungapNormalized2nominal(double normalizedScore)
{
  ...
  return (int32_t)floor(nominalScore);
}
...
int32_t stat_ungapEvalue2nominal(double evalue)
{
  ...
  return (int32_t)ceil((stat_log2 * normalizedScore + stat_ungapLogK)
         / stat_ungapLambda);
}
...
{
  return (int32_t)ceil(((stat_log2 * normalizedScore + stat_gapLogK)
         / stat_gapLambda) - 0.5);
}
...
{
  return (int32_t)ceil(log(stat_gapK * stat_effectiveSearchSpaceSize / evalue) / stat_gapLambda);
}
