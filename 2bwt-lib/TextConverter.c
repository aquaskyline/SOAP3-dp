...
void * DNAMapPacked ( const char * inputFileName, unsigned int * textLength, const unsigned int trailerBufferInWord )
{
    ...
    fread ( textLength, sizeof ( unsigned int ), 1, inputFile );
    unsigned int wordToProcess = ( *textLength + 64 - 1 ) / 64 * 4 + ( ( ( trailerBufferInWord + 3 ) / 4 * 4 ) * 4 );
...
void DNAFreePacked(void* packedDNA, const unsigned int textLength, const unsigned int trailerBufferInWord, char isShareIndex) {
