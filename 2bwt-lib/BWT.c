...
BWT *BWTCreate(MMPool *mmPool, const unsigned int textLength, unsigned int *decodeTable) {
    ...
    bwt->cumulativeFreq = (unsigned int*) MMPoolDispatch(mmPool, (ALPHABET_SIZE + 1) * sizeof(unsigned int));
    ...
    bwt->occValueMajor = (unsigned int*) MMPoolDispatch(mmPool, bwt->occMajorSizeInWord * sizeof(unsigned int));
...
BWT *BWTLoad(MMPool *mmPool, const char *bwtCodeFileName, const char *occValueFileName, 
    ...
    if ( isShareIndex )
    {
        ...
        bwt->occValue = ( unsigned int * ) mmap ( NULL, ( sizeof ( unsigned int ) * ( bwt->occSizeInWord + OCC_PAYLOAD_OFFSET ) ), PROT_READ, MAP_SHARED | MAP_NORESERVE, occValueFD, 0 );
        bwt->occValueMajor = ( unsigned int * ) mmap ( NULL, ( sizeof ( unsigned int ) * ( bwt->occSizeInWord + bwt->occMajorSizeInWord + OCC_PAYLOAD_OFFSET ) ), PROT_READ, MAP_SHARED | MAP_NORESERVE, occValueFD, 0 );
        int rtVal;
        rtVal = mlock ( bwt->occValueMajor, ( sizeof ( unsigned int ) * ( bwt->occSizeInWord + bwt->occMajorSizeInWord + OCC_PAYLOAD_OFFSET ) ) );
    ...
    bwt->occValueMajor = (unsigned int*) MMUnitAllocate(bwt->occMajorSizeInWord * sizeof(unsigned int));
    ...
    if (saValueFile == NULL) {
        ...
        if ( isShareIndex == 1 )
        {
            ...
            bwt->saValue = ( unsigned int * ) mmap ( NULL, ( ( sizeof ( unsigned int ) * bwt->saValueSizeInWord ) + SA_PAYLOAD_OFFSET ), PROT_READ, MAP_SHARED | MAP_NORESERVE, saValueFD, 0 );
            int rtVal;
            rtVal = mlock ( bwt->saValue, ( ( sizeof ( unsigned int ) * bwt->saValueSizeInWord ) + SA_PAYLOAD_OFFSET ) );
        ...
        {
        bwt->saValue = (unsigned int*) MMUnitAllocate(bwt->saValueSizeInWord * sizeof(unsigned int));
    ...
    if (inverseSaFile == NULL) {
        ...
        bwt->inverseSa = (unsigned int*) MMUnitAllocate(bwt->inverseSaSizeInWord * sizeof(unsigned int));
    ...
    if (cachedSaIndexFile == NULL) {
        ...
        bwt->cachedSaIndex = (unsigned int*) MMUnitAllocate((ALPHABET_SIZE + 1) * sizeof(unsigned int));
        ...
        bwt->cachedSaIndex = (unsigned int*) MMUnitAllocate((numOfCachedSaIndex + 1) * sizeof(unsigned int));
...
void BWTFree(MMPool *mmPool, BWT *bwt, char isShareIndex) {
    ...
    if ( isShareIndex )
    {
        ...
        {
            bwt->occValue -= OCC_PAYLOAD_OFFSET;
            munmap ( bwt->occValue, ( ( sizeof ( unsigned int ) * bwt->occSizeInWord ) + OCC_PAYLOAD_OFFSET ) );
        }

        if ( bwt->occValueMajor != NULL )
        {
            bwt->occValueMajor -= ( OCC_PAYLOAD_OFFSET + bwt->occSizeInWord );
            munmap ( bwt->occValueMajor, ( ( sizeof ( unsigned int ) * ( bwt->occSizeInWord + bwt->occMajorSizeInWord ) ) + OCC_PAYLOAD_OFFSET ) );
        }
        
        if ( bwt->saValue != NULL )
        {
            bwt->saValue -= SA_PAYLOAD_OFFSET;
            munmap ( bwt->saValue, ( ( bwt->saValueSizeInWord * sizeof ( unsigned int ) ) + SA_PAYLOAD_OFFSET ) );
        }
    }
    else
    {
...
void BWTGenerateSaValueOnBoundary(MMPool *mmPool, BWT *bwt) {
    ...
    if (bwt->saValueOnBoundary == NULL) {
        bwt->saValueOnBoundary = (unsigned int*) MMPoolDispatch(mmPool, sizeof(unsigned int) * 2 * ALPHABET_SIZE);
    }
...
unsigned int BWTFullSaValue(const BWT *bwt, unsigned int saIndex) {
