...
BWTInc *BWTIncCreate(MMPool *mmPool, const unsigned int textLength, const float targetNBit,
    ...
    bwtInc->cumulativeCountInCurrentBuild = (unsigned int*) MMPoolDispatch(mmPool, sizeof(unsigned int) * (ALPHABET_SIZE + 1));
...
//DEBUG         for (i=0; i<=ALPHABET_SIZE; i++) {
//DEBUG             printf("bwtInc->cumulativeCountInCurrentBuild[%u]=%u\n",i,bwtInc->cumulativeCountInCurrentBuild[i]);
//DEBUG         }
//DEBUG         for (i=0;i<numChar;i++) {
//DEBUG             printf("relativeRank[%u]=%u\n",i,relativeRank[i]);
//DEBUG         }
...
unsigned int BWTGenerateOccValueToFileFromBwt(const char *bwtFileName, const char *occValueFileName, unsigned int*  decodeTable) {
    ...
    occValueMajor = (unsigned int*) MMUnitAllocate(occMajorSizeInWord * sizeof(unsigned int));
...
void BWTGenerateCachedSaIndex(const BWT *bwt, const unsigned int numOfChar, const char *cachedSaIndexFileName) {
    ...
    cachedSaIndex = (unsigned int*) MMUnitAllocate((numOfPattern+1) * sizeof(unsigned int));
...
void BWTGenerateSaBitmap(const BWT *bwt, const unsigned int numOfChar, const char *saBitmapFileName) {
    ...
    saBitMap = (unsigned int*) MMUnitAllocate(bitmapSize * sizeof(unsigned int));
...
void BWTPrintAndVerifySaValue(const BWT *bwt, FILE *output) {
    ...
    printedValue = (unsigned int*) malloc(sizeof(unsigned int) * (bwt->textLength + 1));
