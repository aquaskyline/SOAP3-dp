...
HSP *HSPLoad(MMPool *mmPool, const char *PackedDNAFileName, const char *AnnotationFileName, const char *AmbiguityFileName, const char * TranslateFileName, const unsigned int trailerBufferInWord,  char isShareIndex) {
    ...
    if (PackedDNAFileName != NULL && PackedDNAFileName[0] != '\0' && PackedDNAFileName[0] != '-') {
        if ( isShareIndex )
            hsp->packedDNA = ( unsigned int * ) DNAMapPacked ( PackedDNAFileName, &hsp->dnaLength, trailerBufferInWord );
        else
...
void HSPFree(MMPool *mmPool, HSP *hsp, const unsigned int trailerBufferInWord, char isShareIndex) {

    if (hsp->packedDNA != NULL) {
        DNAFreePacked(hsp->packedDNA, hsp->dnaLength, trailerBufferInWord, isShareIndex);
    }
...
unsigned int HSPParseFASTAToPacked(const char* FASTAFileName, const char* annotationFileName, const char* packedDNAFileName, const char* ambiguityFileName, const char * translateFileName,
    ...
    int ambiguityAllocated =  256;
    unsigned int totalDiscardedChar = 0;
        ...
        /*while (!feof(FASTAFile) && c != '>') {
            ...
            if (c != '\n' && c != '\t') {
                ...
                if (ambiguityCount[charMap[c]] == 1) {
                    ...
                    numChar++;
                } else {//Char != A C G T...can be anything else on Earth!!
                    nCount++;
                }
            }
            c = (char)getc(FASTAFile);
...
//Char != A C G T...can be anything else on Earth!!
    ...
    //printf("%u\n",numAmbiguity);
    //printf("Discarded %u ambiguous characters.\n",totalDiscardedChar);
    //printf("%u total characters in the sequence file.\n",totalNumChar);
    ...
    while(j<numAmbiguity && k<numSeq) {
        if (ambiguity[j].startPos<seqOffset[k].startPos) {
            ...
            unsigned int ambBlkLength = ambiguity[j].rightOfEndPos - ambiguity[j].startPos + 1;

            //Write Ambiguous Segment Flag
            unsigned int index = ambiguity[j].startPos;
            ambiguityMap[index/GRID_SAMPLING_FACTOR]+=1;
            
            //Write the new translate block to store this ambiguous block
            translate[i].startPos=ambiguity[j].startPos;
            translate[i].chrID=k;

            unsigned int seqUnambStartIdx = 0;
            if (k>0)
                seqUnambStartIdx = seqActualOffset[k-1].startPos;

            // Mind that the correction is going to be subtracted out of
            // the retreived text position
            translate[i].correction = seqUnambStartIdx - ambBlkLength - 1;
            
            j++;
            i++;
        } else if (ambiguity[j].startPos>seqOffset[k].startPos) {
            //Write Artificial Chromosome Start Flag
            unsigned int index = seqOffset[k].startPos;
            ambiguityMap[index/GRID_SAMPLING_FACTOR]+=1;

            //Write the new translate block to store this ambiguous block
            //This block is a logical block for chromosome boundary
            ...
            // If the ambiguous block occurs at the same position as
            // the chromosome boundary then the articifical flag is not generated.
            // Skipping by increasing k.
            k++;
        }
    }
...
unsigned int HSPPackedToFASTA(const char* FASTAFileName, const char* annotationFileName, const char* packedDNAFileName, const char* ambiguityFileName, const char* translateFileName) {
    ...
    hsp = HSPLoad(NULL, packedDNAFileName, annotationFileName, ambiguityFileName,translateFileName, 1, 0);
...
void HSPPrintAlignment(MMPool *mmPool, FILE *outputFile, MMPool *alignmentPool, const GappedHitListWithAlignment* gappedHitList, const int numOfHit, 
    ...
    for (i=0; i<numOfHit; i++) {
        ...
        case OUTPUT_PAIRWISE :
            ...
            if (contextInfo[contextNum].reversed) {
                queryIndex = queryPatternLength + 1 - gappedHitList[i].posQuery - gappedHitList[i].lengthQuery;
                unsigned int tp = gappedHitList[i].posText + gappedHitList[i].lengthText - 1;
                
                unsigned int approxIndex = tp>>GRID_SAMPLING_FACTOR_2_POWER;
                unsigned int approxValue = hsp->ambiguityMap[approxIndex];
                ...
                queryIndex = gappedHitList[i].posQuery + 1;
                unsigned int tp = gappedHitList[i].posText;
