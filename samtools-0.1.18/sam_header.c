...
static HeaderLine *sam_header_line_clone(const HeaderLine *hline)
{
    ...
    while (tags)
    {
        HeaderTag *old = (HeaderTag *) tags->data;

        HeaderTag *newTag = (HeaderTag*) malloc(sizeof(HeaderTag));
        newTag->key[0] = old->key[0];
        newTag->key[1] = old->key[1];
        newTag->value  = strdup(old->value);
        out->tags = list_append(out->tags, newTag);
...
char *sam_header_write(const void *_header)
{
    ...
    while (hlines)
    {
        ...
        HeaderLine *hline = (HeaderLine *) hlines->data;
        list_t *tags = hline->tags;
        while (tags)
        {
            HeaderTag *tag = (HeaderTag *) tags->data;
...
void *sam_header2tbl(const void *_dict, char type[2], char key_tag[2], char value_tag[2])
{
    ...
    while (l)
    {
        HeaderLine *hline = (HeaderLine *) l->data;
...
char **sam_header2list(const void *_dict, char type[2], char key_tag[2], int *_n)
{
    ...
    while (l)
    {
		...
		if (n == max) {
			max = max? max<<1 : 4;
			ret = (char **) realloc(ret, max * sizeof(void*));
		}
		ret[n++] = key->value;
...
void *sam_header_merge(int n, const void **_dicts)
{
    ...
    for (idict=1; idict<n; idict++)
    {
        ...
        while ( tmpl_hlines )
        {
            ...
            if ( !inserted )
                out_dict = list_append(out_dict, sam_header_line_clone((const HeaderLine *) tmpl_hlines->data));
